...
func TestGetUser(t *testing.T) {
	...
	t.Run("should hit the cache first and if not exists it sets the user on the cache", func(t *testing.T) {
		...
		mockCacheStore.On("Get", int64(101)).Return(nil, nil)
		...
		// mockCacheStore.AssertNumberOfCalls(t, "Get", 2) // Would be failed, because hit more than 1
		mockCacheStore.AssertNumberOfCalls(t, "Get", 1) // Would be passed, because hit 1 time
